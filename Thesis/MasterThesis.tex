...
There is a lot more \ga\ transitions visible from the present experiment in \autoref{fig:gam_dcB} compared to the previous experiment. 
...
\begin{table}[htb!] 
    ...
    $E$ refers to the energy, $J^\pi$ is the spin and parity, $I_\gamma$ is the \textcolor{red}{??? \ga\ intensity ???} and $\sigma \lambda$ is the multipolarity. 
...
The level scheme in \autoref{fig:levels} is a visualization of the known \ga\ transitions in \autoref{tab:gamma_trans}.
It displays all the known levels up to the $8^+$ state at 2970 keV. 
...
In addition to all the transitions visible in the \ga\ spectrum, it is expected to see a transition from an unknown $3^+$ state which should transition into the $2_2^+$ state.
...
\textcolor{red}{Show coincidence spectra to convince the reader that there is a transition at approximately 845 keV, which is a very good candidate for the missing member of the \ga-band. This new transition and new state will really help to clarify the role of triaxiality.}
