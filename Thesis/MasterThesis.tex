...
\textbf{\textcolor{red}{SKRIV OM DE FORSKJELLIGE REGIONENE I PLOTTET UNDER!}}

\autoref{fig:EFBQ2} has some interesting regions.

\begin{figure}[ht]
	\centering
	% ParticlePlot.cpp
	% plot_single_front_back_energy("setup_Sm.txt", 2, "user")
	\includegraphics[width=\textwidth]{../Plots/plotting/E_f_b_Q2-user-drawing.png}
	\caption{Back energy vs. front energy for quadrant 2 of the CD. The $z$-axis is in log scale.}
	\label{fig:EFBQ2}
\end{figure}

There are still visible blobs parallel to the line, but these might come from pile-up in the detector. 

In several spectra, there was a second peak on the right side of the Sm peak like the one in quadrant 4 of \autoref{fig:BDS_R1}.
These arises from the charge sharing of neighboring segments. 


In \autoref{fig:cal_CD} the energy of every strip is shown for each quadrant separately.
...
\subsection{Time structure and calibration}\label{ssec:time_cal}
...
From \autoref{ssec:user_cal}, the output from \texttt{Q4S.sh} using \texttt{TreeBuilder}: WeightPR: 0.75.
