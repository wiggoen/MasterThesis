...
The analysis code for MIniball is named MiniballCoulexSort and is available on GitHub at \url{https://github.com/Miniball/MiniballCoulexSort}. The main steps of using it is outlined in the README.md file. 


Data from Miniball comes in the form of .MED-files (Miniball Event Data). In order to analyze this data in ROOT the first part of the sorting is just to convert the MED-files into ROOT-files, and the main file for this is called \textbf{MedToRoot.cc}. The process of how to run the 



\section{Scripts}
In order to not copy and paste the sorting command in the terminal for every data file, I made a bash script to do this. The script is called \textbf{M2R.sh} found in the folder Scripts/sorting on GitHub \textbf{\textcolor{red}{(NOT UPLOADED YET)}}.
...
Simulation fit $\rightarrow$ AQ4\_fit() $\rightarrow$ 
