...
ISOLDE is a Radioactive Ion Beam (RIB) facility at CERN in Meyrin, Switzerland. \autoref{fig:accelerators} shows the CERN accelerator complex, where ISOLDE is located beside the Proton Synchrotron Booster (PSB), in the lower right marked by a green box. The acronym ISOLDE stands for Isotope Separator On Line DEvice. The facility can produce over 1000 different radionuclides to be used in a wide variety of experiments in nuclear physics, atomic physics, solid state physics, life sciences and fundamental interactions. Experiments have been performed at ISOLDE since 1967 and since 2001 experiments with post-accelerated RIBs have been conducted. The High Intensity and Energy upgrade (HIE-ISOLDE) have made it possible to deliver energies up to 10 MeV/$u$ in 2018 \cite{HIE-ISOLDE, ISOLDE-web, ISOLDE-facility}. The present experiment was one of the first Miniball experiments with the former newly upgraded superconducting LINear ACcelerator (LINAC), the HIE-ISOLDE LINAC. \textcolor{red}{Jeg har ikke funnet tall på hvor høyt den gikk, men en plass mellom 5-7.5 MeV/u.}
...
When the beam isotopes de-excite, they emit \ga-rays, which \textcolor{red}{??? can be / are ???} observed to have large Doppler shifts.
...
\textcolor{red}{??? Nytt avsnitt ???} In this experiment, a neutron-deficient RIB was requested, and a primary target of tantalum (Ta, $Z = 73$) was chosen.
...
After RILIS has selected Sm, we have a continuous beam of Sm ions at an energy of 60 keV. 
The primary target is on a 60 kV high voltage platform \textcolor{red}{??? Hva betyr dette? Er det viktig? ???} \cite{ISOLDE-web, TIF}. 
...
\textcolor{red}{Skal jeg putte inn en tegning av esperiment-hallen? evt. i appendix? Og/eller bare fjerne info om HRS under her? HRS blir ikke brukt, og jeg har ikke nevnt noen av alle de andre oppsettene i eksperiment-hallen.} \newline
...
Even though the HRS have a high mass resolving power, $M/\Delta M > 5000$, it is not sufficient resolving power to separate the isobars, which is why RILIS and the GPS had to be used instead.
...
REXEBIS releases the beam with a certain energy through another mass separator before guiding the RIB into the HIE-ISOLDE LINAC. This second mass separator removes residual gas (beam contaminants) from the beam released from REXEBIS \cite{HIE-ISOLDE}. 
...
As the \Sm\ particles from the beam fly towards the particle detector, before they are detected, they de-excite by emitting \ga-rays, which are detected by the \ga-detectors.
...
Unfortunately, there was a finger print on the target, implying \textcolor{red}{??? a / an additional ???} contamination (probably carbon).
...
The first excited state is an octupole vibration with an energy of 2615 keV, a half-life of $T_{1/2} = 16.7$ ps and a \textcolor{red}{??? spin / angular momentum ???} and parity of $J^\pi = 3^-$.
...
The TDC keeps track of the time of registered particle-\ga\ \textcolor{red}{??? and particle-\ga-\ga\ ???} coincidences. 
...
\begin{figure}[htb]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Detected particle events. The upper curve marked inside the full lines is the Sm and the lower curve marked inside the dotted lines is the Pb. The contaminant is under the Pb area.}
		\label{fig:part_wcut}
	\end{subfigure}
	\hfill 
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Simulated kinematics. The upper curve is the Sm and the lower curve is the Pb.}
		\label{fig:kinsim}
	\end{subfigure}
...
\begin{figure}[htb]
	...
	\begin{subfigure}[b]{0.49\textwidth}
		\centering
    		...
    		\caption{}
	\end{subfigure}
	\caption{On the left side is a histogram of the CD debugging. The IDs on the x-axis is explained by the table on the right side. The IDs show the number of strips fired at the front and back side of the CD.}
	\label{fig:CD_debug}
\end{figure}
...
\autoref{fig:HPGe} shows a sketch of the Miniball cluster geometry and \autoref{tab:Geo} gives the angles and distance of the different clusters.
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\label{fig:BDS_B1}
	\end{subfigure}
	\caption{Broken detector strips.}
	\label{fig:BDS}
\end{figure}
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Online calibration. Back energy vs. front energy for quadrant 4 of the CD.}
		\label{fig:bump}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{User calibration. Back energy vs. front energy for quadrant 4 of the CD. A similar figure to figures 39 and 40 in \cite{Rosiak}.}
	\label{fig:EFBQ4}
	\end{subfigure}
	\caption{\textcolor{red}{Fiks figur-tekst siden den er feil... henvis til riktig figur.} (a) The second peak of \Sm\ can be explained by region 2 from (b). For more information, see text.}
	\label{fig:FBE}
\end{figure}
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Front strip 2 in quadrant 4. At higher energies, there is a double-peak structure of \Sm.}
		\label{fig:th_f}
	\end{subfigure}
	\hfill 
	\begin{subfigure}[t]{0.49\textwidth}
		...
		\caption{Back strip 6 in quadrant 4.}
		\label{fig:th_b}
	\end{subfigure}
