...
Supervisors Andreas Görgen and Katarzyna Hady{\'{n}}ska-Kl{\c{e}}k
...
\begin{figure}[ht]
	...
	\caption{The Coulomb excitation setup at ISOLDE (experiment code: IS558). Adapted from \cite{Klintefjord}.}
	\label{fig:Coulex}
\end{figure}
...
\chapter{Data calibration and analysis}  
...
\texttt{MedToRoot} outputs four files with the naming convention
\begin{itemize}
...
\subsubsection{\textcolor{red}{TODO: Run timing on M2R.sh}}


\section{Sorting and event building}

\textit{140Sm\_208Pb\_pos6\_0xy\_OnBeam.root} 


\section*{\textcolor{red}{Skriv om kalibrering og sortering samtidig, siden det henger sammen i "steg 2". Sjekk notater og tilbakemeldinger lengre ned for tips!}}
...
\section{Calibration}
\subsection{Calibration of the particle detector}

Assuming a linear correlation between the energy $E$ of the particle and the channel number $n$ of the ADC, we get
\begin{equation}\label{eq:Ego}
	E = g \cdot n + a
\end{equation}
where $a$ is the offset in keV and $g$ is the gain in keV/$n$. The offset $a$ can be expressed easily from \autoref{eq:Ego} as 
\begin{equation}\label{eq:offset}
	a = E - g \cdot n 
\end{equation}
To find the gain $g$, we need at least two measuring points, e.g. the peak energy of Sm and the peak energy of Pb for a given angle. The relationship can be written as 
\begin{equation}\label{eq:gain}
	g = \frac{E_{\text{Sm}} - E_{\text{Pb}}}{n_{\text{Sm}} - n_{\text{Pb}}}
\end{equation}

On the front side of the CD we only have two measuring points. By doing a Gaussian fit, we can get the centroids (channels of the peaks) for both Sm and Pb. 

On the back side of the CD we have two peaks per secular strip that we can fit. 
...
\subsection{\textcolor{red}{Notes to section}}
...
\section{\textcolor{red}{Data and sorting notes}}
