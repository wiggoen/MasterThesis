...
\end{equation}
where $B(E \lambda)$ is the reduced transition probability \cite{EE-Coulex}.

\subsubsection*{\textcolor{red}{Rediger under her..}}

\autoref{cross_E}: \textcolor{blue}{"In this expression $B(E \lambda; I_i \rightarrow I_f)$ is the reduced transition probability associated with the radiative transition of multipole order $E \lambda$. It is related to the nuclear matrix elements of the electric multipole operator by the formula"} \cite{EE-Coulex}
...
% $\sigma \lambda$ multipolarity, $\sigma \in \{ E, M \}$.
\cite{Niedermaier, NaR, EE-Coulex}
...
Magnetic excitation: \cite{EE-Coulex}

\textcolor{blue}{"The total magnetic cross section is conveniently written"}
...
the formulas are very similar, but notice the $1/c$ in the first fraction. In addition there are the switching of $E$ to $M$.
...
\end{align}
where $d_{min}$ is a way to approximate the distance of closest approach.
$d_C$ is the Coulomb interaction distance.
$d_s = 5$ fm is an additional safety distance.
$R_b$ and $R_t$ is the radii of the beam and target nuclei respectively, 
$A_b$ and $A_t$ is the mass number of the beam and target nuclei, respectively \cite{RBass, Cline1986}.
