...
The differential cross section, $d\sigma$, for Rutherford scattering is given by 
\begin{equation}
	\frac{d\sigma}{d\Omega} = \left( \frac{Z_b Z_t e^2}{8 \pi \varepsilon_0 m v_0^{' 2}} \right)^2 \cdot \frac{1}{\sin^4 \left( \frac{\theta_b^{'}}{2} \right)}
\end{equation}
where $d\Omega$ is the solid angle, $Z_b$ and $Z_t$ is the proton number of the beam and target nuclei respectively, $e$ is the elementary charge, $\varepsilon_0$ is the electric permittivity in vacuum, $m$ is the mass of the beam particle, $v_0^{'}$ is the velocity of the beam particle in the CM frame and $\theta_b^{'}$ is the beam particle scattering angle in the CM frame \cite{Krane}.
...
\begin{equation}
	d \geq d_{min} = R_b + R_t + d_s = 1.25 (A_b^{1/3} + A_t^{1/3}) + 5~\text{[fm]}
\end{equation}
...
Half-distance of closest approach, $a$, in a head-on collision
\begin{equation}
	 a = \tfrac{1}{2} d = 0.72 \cdot \frac{Z_b Z_t e^2}{\frac{1}{2} m v_0^2} \cdot \frac{A_b + A_t}{A_t} 
\end{equation}

%The kinetic energy, $E^{'}$, in the CM frame is given by
%\begin{equation}
%	E^{'} = \frac{A_t}{A_b + A_t} E_{b}
%\end{equation}
%where $A_b$ and $A_t$ is the mass number of the beam and target nuclei and $E_{b}$ is the kinetic energy of the beam particle in the LAB frame \cite{Niedermaier}.
...
\begin{equation}
	d = a \left( 1 + \frac{1}{\sin \left( \frac{\theta_b^{'}}{2} \right)} \right)
\end{equation}
...
\begin{align}
	...
	&= 1.44 \cdot \frac{A_b + A_t}{A_t} \cdot \frac{Z_b Z_t}{1.25 (A_b^{1/3} + A_t^{1/3}) + 5} 
\end{align}
where ...
\cite{Klintefjord}
\textcolor{red}{Hvor kommer faktoren 0.72 fra?}





One requirement in the semi-classical approach is that the de Broglie wavelength, $\lambdabar$, must be small compared to the impact parameter $b$.

"The Sommerfeld parameter measures the strength of the Coulomb interaction:"
\begin{equation}
	\eta = \frac{d}{2\lambdabar} = \frac{Z_p Z_t e^2}{\hbar v}
\end{equation}
where .... $v$ is the relative velocity of the nuclei in the CM frame.

"If the Sommerfeld parameter is significantly larger than unity ($\eta \gg 1$), one may describe the relative motion of the particles by classical hyperbolic orbits."

"The Sommerfeld parameter $\eta$ relates half the distance of closest approach to the de Broglie wavelength of the projectile, and for $\eta \gg 1$, the semi-classical approximation is fairly accurate." \cite{Cline1969}
