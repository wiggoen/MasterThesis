...
A normalization of the $B(E2; 0^+ \rightarrow 2^+)$ value for \Sm\ to the well-known $B(E2)$ value for the Mo target, that is the transition strengths in the excited $^{94}$Mo target, was performed in order to extract the other $B(E2)$ values for \Sm.
...
\textcolor{red}{Skal jeg gi $B(E2, 0_1^+ \rightarrow 2_1^+$) verdien eksplisitt?}
...
Another condition is that $\mathbf{R}$ should be invariant under reflection and rotation of the coordinate system, that is, $R$ should be unchanged by transformations of the coordinate system.
...
It is of course also possible that the target nucleus gets excited, or that there is no excitation, that is elastic scattering. 
...
By using \texttt{AQ4Sort}, every single front strip and back strip combination, that is every pixel of the CD, can be viewed. 
...
The table in \autoref{fig:CD_debug_table} explains the different debug IDs, that is lists the $x$-axis of \autoref{fig:CD_debug}. 
...
The code yields debug ID 20 when no particle can be found, that is there is no energy registered in either the front or the back strips. 
...
The peak values, that is the time offsets, may change depending on the amount of data sorted.
...
Then, the \ga\ histograms would have been analyzed by means of ROOT using the \texttt{GammaPlot.cpp} script.
...
After the calibration coefficients and the geometry parameters are added to the calibration file, the \texttt{TreeBuilder} have to be re-run, utilizing the updated calibration file.
...
'-r' (reject)
\end{lstlisting}
\textcolor{red}{As it turns out, the default and singles method are the same.}
...
\autoref{fig:part_wcut} shows the cuts of the beam and target for the detected particle events as the solid and dotted lines. 
These graphical cuts are used in the Doppler correction of the \ga-rays. 
\textcolor{red}{comment on how much the light contaminant bothers you?} 
...
\begin{figure}[htb]
	...
	On lower angles it is difficult to separate the lower solid line from the upper dotted line, but this is visible at higher angles.
...
\begin{figure}[ht]
	...
	\caption{Beam gated prompt, Doppler corrected \ga-rays. Core ID 20 is removed, see \autoref{sec:BDS}. \textcolor{red}{Perhaps add a similar plot for the source data (without DC of course)?}}
	\label{fig:B_dcB_cid}
\end{figure}
...
\textcolor{red}{Minn leseren på hvorfor  vi trenger å gjøre Doppler igjen?}\newline
...
From the previous experiment, the $B(E2)$ values at low-lying states was obtained.

From the present experiment it is expected that excited sates up to $8^+$ is populated. 
\autoref{tab:gamma_trans} displays the known \ga\ transitions for excited \Sm\ up to the $8^+$ state.

\autoref{fig:levels} displays the level scheme for all known levels of \Sm\ up to the $8^+$ state at 2970 keV. 
...
Other reasons might be that the excitation probability for the states are low, and thereby the states are not reached, or that the transition intensity is so low that there are almost no counts.
...
\autoref{fig:gam_dcB} ...
The transition probabilites, $B(E2)$, are relatively strong in the ground state band. 
...
\begin{figure}[htb]
	...
	\caption{Total statistics for \ga-rays, background subtracted, Doppler corrected for scattered projectile. The blue curve is sorted with singles, and the red curve is sorted with add-back. The green arrows marks some of the energy levels in the level scheme, this is a combination of two or more \ga\ transitions.}
	\label{fig:gam_dcB}
\end{figure}
...
\begin{figure}[htb]
	...
	\caption{Level scheme for \Sm\ based on \cite{Klintefjord, NNDC-levels}. The energies are given in keV and are rounded to the nearest integer.}
	\label{fig:levels}
\end{figure}
...
As seen in the output of the \texttt{CLXAna} run in \label{ssec:gamma}, the cut file \textit{outputfile.root} is provided as  
\begin{lstlisting}[language=]
