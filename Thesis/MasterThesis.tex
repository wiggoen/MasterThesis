...
A lifetime measurement of the $2_1^+$ state also gave the $B(E2)$ value.

\begin{table}[htb] 
    \centering 
    \caption{Known $B(E2)$ values with corresponding errors, from \cite{Klintefjord2016, BelloGarrote2015}. TNA (LNA) is an acronym for Target (Lifetime) Normalization Approach. LM is an acronym for Lifetime Measurement.}
	\input{Table-data/BE2.tex}
	\label{tab:BE2}
\end{table}

From \cite{Klintefjord2015} the spin values of the 990 keV and 1599 keV levels was corrected to $2^+$ and $0^{(+)}$, respectively.
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		%\label{fig:???}
	\end{subfigure}
	\caption{\textbf{(a)-(d)} shows when the beam particle hits the CD in the LAB angle, while \textbf{(e)-(f)} shows when the target hits the CD in the LAB angle. Combined they cover the angle range of the beam in the CM frame $\theta_b^{'} \in [36.6^\circ, 136.0^\circ]$.}
