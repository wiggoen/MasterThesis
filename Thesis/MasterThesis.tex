...
%The spin and parity of the 990 keV \cite{Firestone} and 1599 keV states was originally assumed to be $0^+$ and $2^+$, respectively. 
...
In this thesis, the raw data from the experiment have been presorted and used to produce spectra to calibrate all detectors.
...
There still remains a lot of work to properly document the codes \textcolor{red}{(outlook)}.



\textcolor{blue}{For summary and outlook:}

You have worked on the calibration of the data, performed simulations (kinematics, energy loss etc) in the process. It is also an important job to test and double-check everything. Some silicon strips for ex. had to be excluded from the data. At the end you have produced a set of fully calibrated data, and with the kinematic reconstruction you could apply the Doppler correction. Th quality of the spectra is very high, which demonstrates that your calibration is good. You have produced a set of singles spectra as a function of scattering angle which is now ready to be further analysed to obtain the coulex cross sections. You have also produced gamma-gamma matrices which could be analysed to search for new transitions and extend the level scheme.

From the coincidence analysis you found a candidate for the 3+ state of the gamma band, which was previously unknown. This is an important result that will help to better understand the structure of 140Sm. The new state will be a central aspect of the publication that we are going to write about the experiment.

It is always possible to improve a calibration. If you have concrete suggestions of what could be improved, you can of course mention these. My feeling is that it would take a major effort to make even small improvements, so most likely we will continue further with the GOSIA analysis with the spectra that you have produced.

The only calibration that is still missing is a proper efficiency calibration. This will be done by using the Ba and Eu source data.

With your spectra and the efficiency calibration it is possible to extract the intensities for all transitions for all 23 angles. This is the experimental input into GOSIA. Other experimental inputs are the transition strengths of the $2_1^+ \rightarrow 0_1^+$, $4_1^+ \rightarrow 2_1^+$, and $2_2^+ \rightarrow 2_1^+$ from the previous experiment with their uncertainties, and a few gamma-ray branching ratios that are known. Another input that is needed for GOSIA is the level scheme (including the new state). With GOSIA we will perform a chi-square minimization for find the set of EM matrix elements that best reproduces the gamma-ray intensities and other experimental constraints.

Your data set is one of the most detailed ever produced at ISOLDE, with excellent statistics. Your statistics is so good that you could subdivide the data into 23 angular ranges. This means you have many more data points than unknown variables (=matrix elements) in the chi-square fit. We can therefore expect to obtain all transition matrix elements with good precision, and also several diagonal matrix elements (quadrupole moments).

The final step is then to compare the B(E2) values and quadrupole moments with theoretical calculations, similar to what we did in Malin's paper. (Beyond mean-field calculations, shell model, interactive boson model - you haven't discussed these earlier, so I wouldn't say much about the theoretical calculations.)

\textcolor{blue}{- - - - - summary and outlook - - - - -}
