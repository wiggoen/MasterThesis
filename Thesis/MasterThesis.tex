...
Each quadrant of the CD is connected to a Time to Digital Converter (TDC). The TDC keeps track of the time of registered particle-\ga\ coincidences. 
...
The MADCs, like the DGFs, record an energy and a timestamp with 25 ns ticks. 

The gate for the MADCs and the common stop for the TDCs is generated for each quadrant of the CD independently.

The multiplicity output of the DGFs is used to generate the gamma signal, which is used to make the particle-gamma coincidence.
...
We start the off window 60 $\mu$s after the end of readout (obtained from the disappearance of the DAQ dead signal) to allow the ADCs and TDCs time to start.
