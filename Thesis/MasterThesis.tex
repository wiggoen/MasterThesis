...
As late as 1932, the neutron was discovered by Chadwick. 
...
The magic numbers in the nuclear shell model are for the protons and neutrons individually
\begin{align*}
	Z &= 2, ~8, ~20, ~28, ~50, ~82 \\
...
Extensive descriptions of the liquid drop model and the shell model can be found in \cite{Krane}.
...
A similar experiment with \Sm\ was conducted in 2012 at CERN-ISOLDE.
The experiment code was IS495 with the title Coulomb excitation of \Sm.
The beam energy was 2.85 MeV/u and the secondary target was $^{94}$Mo with a thickness of 2 mg/cm$^2$. 

The goal of the experiment was to deduce the $B(E2)$ values and quadrupole moments of low-lying states in neutron-deficient \Sm\ using multi-step Coulomb excitation.

Out of that experiment (and another experiment) came a PhD thesis by Malin Klintefjord  \cite{Klintefjord} with three papers \cite{Klintefjord2015}, \cite{Samorajczyk2015} and \cite{Klintefjord2016} on \Sm.

\subsubsection*{\textcolor{red}{Rediger her...}}


The old REX-ISOLDE post-accelerator was limited to 2.8 MeV/u, which gave a low Coulomb excitation cross section and a low probability for multi-step excitations.
...
\textcolor{red}{From Klintefjord:\newline
Coulomb excitation was used to extract previously unknown transition strengths between low states in the neutron deficient \Sm\ nucleus. 
The amount of statistics collected allowed for studying the \ga\ intensities as a function of particle scattering angle, thereby gaining sensitivity to the quadrupole moment of the first excited $2^+$ state.
In the first data analysis approach applied on this data, normalization to known transition strengths in the excited $^{94}$Mo target was used. 
With the independent RDDS measurement of the lifetime of the $2_1^+$ state, performed during the time of the analysis, this spectroscopic information could serve as normalization in the second approach. \newline
... 990 keV state...\newline
The results indicate a weak quadrupole deformation with strong triaxiality and \ga-softness. \newline
Outlook: In the COULEX experiment of \Sm\ using 2.85 MeV/u beam energy, excited states up to 1256 keV were populated. 
The recent upgrade of the ISOLDE facility to HIE-ISOLDE is expected to allow for Coulex of \Sm\ at energy up to 4.3 MeV/u. 
In such an experiment, for example the $6_1^+$ state at 2082 keV excitation energy may be populated, hence a deeper understanding of the low-lying nuclear structure in \Sm\ is expected. 
Coulex at that energy may also populate currently unknown $3_1^+$ or $4_2^+$ states. 
Here the staggering of these states can help to distinguish between a triaxial rotor and \ga-softness in the interpretation of the structure in \Sm. 
Furthermore, the measurement of the transition strenght from the $0_3^+$ state would be important to judge how well \Sm\ can be described by the E(5) crit.point symmetry.}
...
The goal of the master thesis is to analyze the data from this experiment.  
...
\Sm\ ($T_{1/2} = 14.82$ min), Lanthanide from the periodic table. rare-earth element. Neutron-deficient nuclei close to the proton drip-line
...
\begin{figure}[htb]
	...
	\textcolor{red}{Skal jeg legge på bilder av fruktene også?}}
...
\end{equation}
and in the special case when $\lambda = 2$, \autoref{eq:Rmid} becomes
\begin{equation}
...
\textcolor{red}{Tror dette må i innledningen på en aller annen måte... + må fjerne noe sånn at jeg slipper å snakke om X(5) og E(5). Det kan en fremtidig PhD-student eller Post-doc gjøre..} \newline
...
$^{140}_{62}$Sm$_{78}$: Just below $N = 82$ shell closure. $Z = 62$: mid shell. 

\Sm\ was thought to have a low-lying $0^+$ state \cite{Firestone}, but this state was shown to be $2^+$ \cite{Samorajczyk2015}. 

Indication of $0^+$ states around 1.5 MeV. 

One object of this experiment is to clarify the nature/structure of these $0^+$ states. 

Shape transition: $^{144}_{62}$Sm$_{82}$ spherical. 
Adding neutrons: transition of $N = 90$ from spherical to prolate deformed implies a shape-phase transition, a so-called $X(5)$ critical-point symmetry.

Taking out neutrons: very neutron-deficient Sm nuclei are also prolate deformed (e.g. Sm-132), but for \Sm: indication for triaxiality/\ga-softness \cite{Klintefjord}, another form of shape-phase transition / critical point behavior, E(5) \cite{Iachello2000}. 
\Sm\ could be one of the best examples of E(5) symmetry, need transition probabilities from higher-lying states to confirm. 
