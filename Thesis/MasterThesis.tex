...
Scripts developed in the present thesis work for the fitting procedures are inspired by scripts written by Ville Virtanen\footnote{Ville Virtanen is a student from University of Jyväskylä.} and Dr. Liam Gaffney\footnote{Dr. Liam Gaffney is a research fellow at ISOLDE, affiliated with Miniball.}. 
...
If the detectors are not properly calibrated, adding spectra from different detectors \textcolor{red}{usikker på hvordan formulere dette. det fører ikke til noe godt, brede topper...}
...
\subsubsection*{\textcolor{red}{Flytt til passende seksjoner...}}
...
In the fitting of the simulated (synthetic) data, a Gaussian function with linear background was applied
\begin{equation}
	g(x) = c + sx + A e^{-\frac{1}{2}\left(\frac{x - \mu}{\sigma}\right)^2}
\end{equation}
where $x$ is the energy centroid, $c$ is the background constant, $s$ is the background slope, $A$ is the amplitude (Gauss constant), $\mu$ is the mean (expected value) and $\sigma$ is the standard deviation (Gauss width). 
...
There is of course also a chance for multi-step excitation at larger angles, so an average excitation energy of 1 MeV is not so bad.
...
In the present experiment there is only 2 data points to calibrate the front side of the CD, the peaks of \Sm\ and \Pb.
...
People at CERN collect data during the campaign and make a common calibration file.
...
When there is a campaign run at ISOLDE, the staff configures a settingsfile and continuously updates it according to new changes in the system setup.
...
The data from these runs are used to make the so-called online calibration file. 
...
For IS558, the present experiment, the settingsfile \textit{MBSettings2017\_CLX\_IS558.dat} and the calibration file \textit{IS558-online.cal} was created.

\textcolor{red}{Planen var å diskutere TreeBuilder (TB) i denne seksjonen, og AQ4Sort i neste, men det ble plutselig litt vanskelig.... Jeg refererte fra kap 4.1 (Data handling) om hvor disse skulle diskuteres. Jeg trenger TB for online cal, og AQ4 for user cal..}\newline
...
\textcolor{red}{er det under her unødvendig?}\newline
...
It is very difficult to determine the centroid of the \Pb\ peak displayed in \autoref{fig:CD_cal_difficult}. 
Such flat peaks as this are challenging to identify correctly by the automatic software.
...
One should be careful to inspect the quality of the fit on a linear scale, as it is more difficult to see deviations on a logarithmic scale. \textcolor{red}{Må jeg splitte figuren siden jeg ikke refererer til (a) og (b) enda? Bare tenkte de var greie å ha sammen..}
...
\textcolor{red}{Finne en passende plassering.}
