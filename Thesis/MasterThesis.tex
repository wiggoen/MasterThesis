...
Both the higher $Z$ of the target and the higher beam energy contribute to the higher excitation probability of \Sm.  
A higher excitation energy and a higher spin is reached, as seen by the excitation of states up to $\sim$3 MeV.
From \autoref{fig:gam_dcB}, we see the population of the $8_1^+$ state, whereas the previous experiment only populated up to the $4_1^+$ state. 
This is due to the much higher probability of multi-step excitation.
The higher probability for multi-step excitation can also be seen from the relative intensity of the transitions that require two steps ($2_2^+ \rightarrow 2_1^+$ (460 keV) or $4_1^+ \rightarrow  2_1^+$ (715 keV)) relative to the $2_1^+ \rightarrow  0_1^+$ (531 keV) transition in the two experiments.
...
\textcolor{red}{Start fiks - - - - - - - - - -}\newline
\textcolor{ForestGreen}{Jeg tror bare dette var en forklaring til meg, ikke legg til i teksten.}\newline
notion of triaxiality = forestilling om triaxialitet \newline
The fact that the second $2^+$ states is so low indicates that the axial symmetry is broken. 
If you had a stable triaxial shape of the nucleus (in the ground state), you would expect a low-lying $2_2^+$ (and $3_1^+$) state coming from the rotation of the triaxial nucleus. (we talked about this in the nuclear structure course, see f.ex. Davydov and Filippov, Nucl Phys. 8, 237, 1958). A very low $2_2^+$ state would require a stable deformation with gamma near 30 deg. But such cases are rare. A more likely interpretation would be a gamma vibration. In that case you have a deformation with axial symmetry in the ground state, and the $2_2^+$ state is based on the vibration of the shape in gamma direction about the axial equlibrium shape. The energy of the $2_2^+$ state and the $B(E2; 2_2^+ \rightarrow 2_1^+)$ value cannot distinguish between stable triaxial deformation and gamma vibration. The $3^+$ state, on the other hand, is sensitive to these two different types of excitations. This is why the potential new $3^+$ state that we found gives important information on the nuclear shape.

\textcolor{red}{Slutt fiks - - - - - - - - - -}
...
In Warsaw the angular correlation were measured, and the experiment could only conclude that the multipolarity of the $0_2^{(+)} \rightarrow 2_1+$ transition was quadrupole ($\lambda = 2$). 
Because E2 and M3 transitions have the same angular distributions, the experiment could not determine the parity. 
Magnetic excitations are strongly suppressed in Coulomb excitation compared to electric excitations. 
The fact that the $0_2^{(+)}$ state was populated in the present experiment, already reveal that the parity of the state must be positive, i.e. a $=0^+$ state. 
There is no way that a $0^-$ state could be populated from a $2^+$ state in Coulomb excitation.
...
\textcolor{blue}{The new transition places the candidate for the $3^+$ state at 1834 keV.}
...
\textcolor{blue}{No, that is very unlikely to have three gamma rays going into the same detector. In any case would they have to form a cascade, i.e. would have to be all in coincidence with each other.}
...
These intensities were used to obtain the reduced transition probabilities, i.e. the $B(E2)$ values of the specified low-lying states \cite{Klintefjord2015, Klintefjord2016, BelloGarrote2015}.
...
\textcolor{red}{Fiks......}
It is split into 6 parts as an example of the quality of the new measurement.

The silicon detector has 16 rings and consequently covers 16 angular ranges in the laboratory system. 
...
In the next step of the Coulomb excitation analysis, a set of electromagnetic matrix elements will be fitted to the \ga-ray intensities in a chi square procedure. 
