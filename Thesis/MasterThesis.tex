...
In the following sections there will be an introduction of different programs and scripts used in the data calibration and analysis. 
One thing to note here, is that the numbering of the CD rings and strips are different in these programs and scripts. 
...
In order to analyze the data in the ROOT\footnote{ROOT is a data analysis framework written in C/C++ made at CERN.} framework, the first part of the code is just to convert the \textit{.med}-files produced by \MBOU\ into \textit{.root}-files with the program \texttt{MedToRoot}. 
...
(If you are using MeV/cm$^2$, you must change the distance on line 359 to be in the correct units. 
...
\autoref{tab:paths} shows the relative path of programs and scripts used in this thesis.

\begin{table}[ht] 
    \centering 
    \caption{Relative path of programs and scripts.}
