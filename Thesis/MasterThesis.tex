...
As a comparison, the total \ga-ray singles spectrum from the previous experiment is displayed in \autoref{fig:gam_MK}.
From the experiment in 2017 with the \Pb\ target, a lot more excited states are populated and visible in the spectra compared to the experiment in 2012 with the $^{94}$Mo target.
By looking at the relative intensity of $2_2^+ \rightarrow 2_1^+$ (460 keV) compared to $2_1^+ \rightarrow 0_1^+$ (531 keV), we see that the probability for multi-step excitations are much greater with the Pb target.
...
In \autoref{fig:gam_dcB} there is a visible enhanced efficiency for high-energy \ga-rays when using the add-back method. \textcolor{red}{Jeg ser ikke helt hvor mye bedre det blir. Jeg ser at det er tydelig høyere counts rundt 800-1400 keV.}
...
\textcolor{red}{TODO:}
\begin{itemize}
	\item \textcolor{blue}{in addition to all the transitions you see in the spectrum, you would also expect to see a transition from an unknown 3+ state which should feed into the second 2+ states. (The 3+ state is expected as a member of the gamma band-vibrational band, which is expected to be built on the second 2+ state.)}
	\item \textcolor{blue}{In the spectrum of fig.5.1. there is no obvious candidate for a transition from the expected 3+ state. However, the width of the transition at ~840 keV is slightly wider than the other transitions. Gamma-gamma coincidences can be used to investigate if the peak at 840 keV is composed of more than one transition. Then I would show coincidence spectra to convince the reader that there is a transition at ~845 keV which is a very good candidate for the missing member of the gamma band. this new transition and new state, will really help to clarify the role of triaxiality.}
\end{itemize}


\subsubsection*{\textcolor{red}{Rediger her...}}

\textcolor{red}{Discuss all the transitions in \autoref{fig:gam_dcB}, where they are in the level scheme \autoref{fig:levels}, and explain \autoref{tab:gamma_trans}.}


From the previous experiment at ISOLDE, IS495 in 2012, three excited states of \Sm\ were populated, the first (531 keV) and second (990 keV) excited $2^+$ states, in addition to the first $4^+$ (1246 keV) state.
The reduced transition probabilities, i.e. $B(E2)$ values for the specified low-lying states was obtained \cite{Klintefjord2015, Klintefjord2016, BelloGarrote2015}.
...
\textcolor{blue}{ the B(E2) values for the 3 transitions (2x2+, 4+) were measured in the earlier experiment. the precision of the B(E2) values will certainly be improved.}
