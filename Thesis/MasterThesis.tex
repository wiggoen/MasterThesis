...
\chapter{Detector calibration and data analysis}  
...
One program that is mentioned in the Miniball GitHub repository, but not showed how to use, is the \texttt{AQ4Sort}. It is used in the same way as the \texttt{TreeBuilder} script, but it sorts the histograms in another way. 
This program is used before and during the calibration of the detectors, because it gives information about every single ring and every single back strip (the pixels of the detectors). The one thing to note here, is that the numbering of the detector rings and strips are different from the ones used in \texttt{TreeBuilder}. The histograms sorted by \texttt{TreeBuilder} starts counting from 0 and the \texttt{AQ4Sort} starts counting from 1. 
...
\end{itemize}
where the file of interest is the first one. The \textit{OnBeam}-files are the files used in the sorting and event building with \texttt{TreeBuilder} (and/or \texttt{AQ4Sort}).
...
The general idea of the calibration is to make sure that the energy spectra from the detectors have the same physical features. 
...
\subsubsection{\textcolor{red}{Particle detector calibration?}}
Assuming a linear correlation between the energy $E$ of the particle (\textcolor{red}{or \ga?}) and the channel number $n$ of the ADC, we get
...
\subsubsection*{\textcolor{red}{Online calibration}}
Every year there is a campaign at ISOLDE, where the staff configures a settingsfile if there are any changes in the setup system. In addition they run a coctail beam composed of different isotopes on a specific target to make a calibration file. This calibration file is adjusted for each experiment following the campaign period. In this way it is easy to check during the experiment if it is going well and to make preliminary Doppler-corrected \ga-spectra. 
...
Then the OnBeam files from \texttt{M2R.sh} is run through using \texttt{TreeBuilder} in the \texttt{Q4S.sh} script. 
...
\begin{enumerate}
	\item \texttt{Q4S.sh}
	\begin{itemize}
		\item uses either \texttt{AQ4Sort} to make histograms for each pixel of the particle detector (used for calibration) or \texttt{Treebuilder} to build events with a lot of files in one go. 
	\end{itemize}
	\item 
	\begin{itemize}
...
\textit{140Sm\_208Pb\_pos6\_0xy\_OnBeam.root}  \newline
...
Simulation done by kinsim3 \cite{kinsim} + SRIM-2013 \cite{SRIM} \newline
\textcolor{red}{Say something about SRIM (the Stopping and Range of Ions in Matter) files.}

\bigskip

To calibrate the data, we need to know the expected energy of the centroids of the peaks. 
This was done by simulating the experiment in a program called kinsim3. The program is written by Liam Gaffney and the purpose of the program is to simulate the kinematics of the experiment. 
...
\section*{\textcolor{red}{Notes to chapter}}

To get useful information out of the converted .root-files, the \texttt{TreeBuilder} script is used. 
The .root-file(s) and a calibration file is given to the \texttt{TreeBuilder} so it can make event trees that can be used for analyzing the Coulomb excitation events. 



The ADC spectras from the file sorted by \texttt{TreeBuilder} have a naming convention of adc\_q\_s, where $q$ corresponds to the quadrant and $s$ corresponds to the channel. The front energy is saved in adc\_[0-3]\_[0-15] and the back energy from strips 1-12 from all 16 rings (the whole quadrant) is saved in adc\_[0-3]\_[16-27].
...
adc\_0\_0 in the file sorted by \texttt{TreeBuilder} is the same as fE\_Q1\_f1 sorted by AQ4Sort. All the front detectors can be found in the \texttt{TreeBuilder}-sorted file, but when it comes to the back detector, the single pixels from the strips are not shown. These are available through the AQ4Sort sorted file. For the front detectors the histograms adc\_[0-3]\_[0-15] and fE\_Q[1-4]\_f[1-16] are the same. For the back detectors, we have that adc\_[0-3]\_[16-27] and bE\_Q[1-4]\_b[1-12] are the same. In addition in AQ4Sort we can see the different pixels. The histograms fE\_Q[1-4]\_f[1-16]\_b[1-12] shows the front energy of quadrant 1-4 gated on ring 1-16 and back strip 1-12, while the bE\_Q[1-4]\_f[1-16]\_b[1-12] shows the same, only for the back energy. 

\textcolor{Magenta}{Tilbakemelding: \newline 
a little confusing: is it correct that the \texttt{TreeBuilder} sort individual spectra for the front and back strips, but if you want coincidences between front and back ($\rightarrow$ pixels) you use AQ4Sort?
}
