...
Histograms sorted by \texttt{TreeBuilder} starts counting from 0 (outermost ring) to 15 (innermost ring) showed in \autoref{fig:CD-FB}. 
In \texttt{AQ4Sort} it starts from 1 (innermost ring) to 16 (outermost ring). 
...
The general idea of the calibration is to make sure that the energy spectra from the detectors have the same physical features, that the detectors show the same energy distribution for the same kind of particles or \ga-rays hitting the same or the angular similarly place in the detector.
...
Appendix \autoref{ch:cd_sim} shows the simulated energy for each ring of the CD.
...
\subsection{Online particle calibration}
...
The ADC spectras from the file sorted by \texttt{TreeBuilder} have a naming convention of \textit{adc\_q\_n}, where $q$ corresponds to the quadrant and $n$ corresponds to the channel. The front energy is saved in adc\_[0-3]\_[0-15] and the back energy from strips 1-12 from all 16 rings (the whole quadrant) is saved in adc\_[0-3]\_[16-27].
...
\subsection{User particle calibration}
