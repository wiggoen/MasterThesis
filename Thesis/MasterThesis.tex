...
The analysis code for Miniball data is named MiniballCoulexSort and is available on GitHub at \url{https://github.com/Miniball/MiniballCoulexSort}. The main steps of how to download, install and use it is outlined in the README.md file in the GitHub repository. 


Data from Miniball comes in the form of .MED-files (Miniball Event Data). In order to analyze this data in ROOT\footnote{ROOT is a data analysis framework made at CERN.} the first part of the sorting is just to convert the MED-files into ROOT-files, and the main file for this is called \textbf{MedToRoot.cc}. The process of how to run this is 
\begin{lstlisting}[language=sh]
$ MedToRoot -mf data_file.med -sf settings_file.dat
\end{lstlisting}
The settings file is provided by ISOLDE and fits to the experiment.

\section{Scripts}
In order to not copy and paste the sorting command in the terminal for every data file, I made a bash script to do this. The script is called \textbf{M2R.sh} found in the folder Scripts/sorting on my GitHub repository \url{https://github.com/wiggoen/MasterThesis}.
...
To plot data some scripts had to be written. 
...
This was done by simulating the experiment in a program called kinsim3. 

kinsim3 is written by Liam Gaffney\footnote{Fellow at ISOLDE, affiliated with MINIBALL.} and the purpose of the program is to simulate the kinematics of the experiment. It takes into account the Silicon dead layer. 
...
"the source has a thickness of 1.23 mm, which needs to be factored in so that the CD to target distance is the CD to source distance PLUS the source thickness, i.e. 25.78 mm + 1.23 mm =  27.01 mm. This is very close to the 26.98 mm you got from us in August. \textcolor{red}{I think that the source data was reanalysed since the original blog entry, giving the 0.03 mm difference!}" \newline
