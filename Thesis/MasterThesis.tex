...
The acronym ISOLDE stands for Isotope Separator On Line DEvice. ISOLDE is a Radioactive Ion Beam (RIB) facility at CERN in Meyrin, Switzerland.  \autoref{fig:accelerators} shows the CERN accelerator complex \cite{CERN-AC}, where ISOLDE is located beside the Proton Synchrotron Booster (PSB). 


\textcolor{red}{kan gjerne forklare figuren litt mer}
...
A continuous proton beam of 1.4 GeV ($\sim 10^{18}$ protons) from the PSB comes into the ISOLDE facility and collide with a thick production target of tantalum (Ta), producing the elements in the chart of nuclides up to tantalum by fragmentation reactions (basically smashing the Ta target into pieces). Tantalum is not used in all experiments, but it was in this experiment. The production target is chosen from a stable region heavier than the nucleus of interest.


\bigskip

\textcolor{red}{Fra Andreas:} \newline
\textcolor{red}{These bunches are accelerated by the PSB continuously, one bunch every 1.2 s. ISOLDE typically takes 50\% of all proton bunches from the PSB, the rest goes to the LHC (and other experiments).}
