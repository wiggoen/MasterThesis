...
In a head-on collision, $b = 0$ and the particles reach the distance of closest approach $d$. 
...
% a = 0.72 \cdot \frac{Z_b Z_t e^2}{\frac{1}{2} m v_0^2} \cdot \frac{A_b + A_t}{A_t} 
\begin{equation}
...
\end{equation}
where $\Delta E = E_f - E_i$ is the excitation energy difference.
\cite{Niedermaier, NaR}

\textcolor{blue}{"For large values of $\xi$ the reaction becomes adiabatic, hindering the excitation process, while for values of $\xi \rightarrow 0$ the scattering happens as a very prompt event. If the conditions $\eta \gg 1$ and $\xi \rightarrow 0$ are fulfilled, a semi-classical treatment of the Coulomb excitation is a very good approximation. In the present work all presented experiments fulfill these conditions."}
...
The energy is below the Coulomb barrier.
...
In safe COULEX, the safety distance is chosen by the condition
\begin{align}
...
The maximum beam energy in the LAB frame, $E_{b, max}$, is chosen from the special case when $\theta_b = 180^\circ$
\begin{align}
	E_{b, max} 
...
\end{align}
where ...
\cite{Klintefjord, RBass}

\textcolor{red}{Dette blir 4.63 MeV/u for Sm på Pb. Hvordan ble 4.7 MeV/u regnet ut?}


\textcolor{blue}{"Only the CM energy $E_{cm}$ and the relative momentum $P_{cm}$ are available for the reaction. The remainder is translational energy (recoil energy) of the total system"} \cite{NaR}.
\begin{equation}
	E_t = \frac{A_b}{A_b + A_t} E_b
\end{equation}

The kinetic energy, $E^{'}$, in the CM frame is given by
\begin{equation}
...
\textcolor{blue}{"For point like charges, the scattering cross section is given by the Rutherford cross section."}
...
\textcolor{blue}{"If $\xi \ll 1$ the process is sudden. Under this condition, the differential cross section is given as a product of the Rutherford cross section and an excitation probability, $P$."} \cite{NaR}

The differential cross section, $d\sigma_{i \rightarrow f}$, for inelastic scattering of point-like objects from the state $|i\rangle$ to $|f\rangle$ is given by \cite{EE-Coulex} 
\begin{align}
...
$P_{i \rightarrow f}$ is given by
\begin{equation}
	P_{i \rightarrow f} = \frac{1}{2I_i + 1} \sum_{M_i, M_f} | a_{if} |^2
\end{equation}
where $I_i$ is the \textcolor{red}{??? spin / angular momentum ???} of the state $|i\rangle$, $M_i$ and $M_f$ is the angular momentum projections of initial and final states, respectively.
...
total cross section for electric multipole excitation
\begin{equation}
	\sigma_{E \lambda} = \left( \frac{Z_b e}{\hbar v_i} \right)^2 a^{-2 \lambda + 2} B(E \lambda) f_{E \lambda} (\xi)
\end{equation}

where $B(E \lambda)$ is the reduced transition probability.

\begin{equation}
	f_{E \lambda} (\xi) = \int_{\theta_{b, min}^{'}}^{\theta_{b, max}^{'}} \frac{df_{E \lambda} (\theta_b^{'}, \xi)}{d\Omega} ~d\Omega
\end{equation}

\cite{Niedermaier, NaR}

\textcolor{blue}{"$f_{E \lambda}$ is a function depending on $\xi$ and on the multipolarity. For $E2$ transitions and $\xi \ll 1$ the value is close to 1. From the reduced transiton probabilities, one can deduce tje deformation properties. For example, for $\lambda = 2$"} \cite{NaR}.
\begin{equation}
	\sigma \approx \left( \frac{Z_b e}{\hbar v_i} \right)^2 a^{-2} = \left( \frac{mv_i}{Z_t e\hbar} \right)^2
\end{equation}

\bigskip

$m(\sigma \lambda)$; multipole operator. $\sigma \lambda$ multipolarity, $\sigma \in {E, M}$.



\bigskip

\textcolor{blue}{"The relation of the reduced transition probability between the excitation $B(E2 \uparrow)$ and the decay $B(E2 \downarrow)$ of the nuclear state is given by"}
\begin{equation}
...
\begin{equation}
	B(E2; I_i \rightarrow I_f) = \frac{1}{2I_i + 1} | \langle f | \mathcal{M}(EL)| i \rangle |^2 
\end{equation}

\bigskip

\textcolor{blue}{"In case of an E2 transition between ground state $0_1^+$ and the first excited state $2_1^+$, we obtain"}
\begin{align}
