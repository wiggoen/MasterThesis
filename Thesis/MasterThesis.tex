...
One of the goals of the present work is to study the nuclear shape of \Sm. \textcolor{red}{Fyll på med litt stoff...}
...
It is a neutron-deficient nuclei compared to the line of stability.
...
They are at $Z = 80$, just below the proton shell closure, $Z = 82$, and at neutron mid shell.
$^{140}_{~62}$Sm$_{78}$ is just below the neutron shell closure, $N = 82$, with a proton mid shell at $Z = 62$. 
In \Sm, there is an indication of $0^+$ states around 1.5 MeV. 
One objective of this experiment is to clarify the nature/structure of these $0^+$ states. 
...
By adding neutrons to $^{144}$Sm, the deformation changes very rapidly to an elongated (prolate) quadrupole shape at neutron number $N = 90$. 
The transition from spherical to prolate deformed shape at $N = 90$ can be interpreted as a shape-phase transition (with the neutron number as control parameter), and  $^{152}$Sm happens to lie at the critical point of this phase transition \cite{Casten2001}.
...
\textcolor{blue}{perhaps you need to mention that our detector covers the angles from 22-57 deg in the lab, and that it is divided up in rings that correspond to the angles shown in the table. As it is now, it is difficult to understand where these angles come from. You could either move these tables to the experimental chapter, and refer to discussion of kinematics in the theory chapter, or you leave it here, but mention that these are our detector angles, and refer to the discussion of the experimental setup that will follow.}
...
\texttt{M2R.sh} takes the element name as command line argument\footnote{\texttt{M2R.sh} was initially developed to convert \textit{.med}-files from different experiments, so it is fairly simple to expand the script to sort other data sets.} and if no command line arguments are given, the script will print out an usage description.
...
Both electronic and nuclear stopping are modeled, i.e. the interaction of the ions with the electrons in the material and collisions with nuclei.
...
Therefore, the relative intensity between the projectile and target peaks are not the same as in the experiment, but the simulated energies are realistic.
...
\texttt{TreeBuilder} produces spectra for each detector strip, i.e. for each ring on the front side and for each sector on the back side. 
...
\autoref{fig:FBE} shows the back vs. front energy for the one quadrant of the CD for both online and user calibration.
...
Every ADC channel for the CD needs calibration coefficients.
...
For example gated on ring 1, 8 and 16. 
...
At least two measuring points are needed to calibrate one strip of the CD.
...
From \autoref{eq:offset} and \autoref{eq:gain} we have more specifically
...
\texttt{kinsim3} delivers the energy centroids in \autoref{eq:GPandA}, while \texttt{AQ4Sort} delivers the channel numbers.
These two data sets are analyzed using ROOT through different functions in the \texttt{ParticlePlot.cpp} script. 
Information about the range of the peaks and guesses of the centroids of Pb and Sm is written into input files used in \texttt{ParticleFit.cpp}. 
Here, the automatic fitting collects the centroids of both data sets and write them into output files which is used as input in \texttt{particle-calibration.py}. 
...
To visualize plots after a new calibration, ROOT or \texttt{ParticlePlot.cpp} is used. 
...
The user calibration file is basically the online calibration file without the innermost ring.
...
\begin{figure}[htb]
	...
	\textbf{(a)} Quadrant 1 using online calibration.
	\textbf{(b)} Quadrant 4 using user calibration. The marked regions are explained in \autoref{ssec:DPS} and are similar to figures 39 and 40 in \cite{Rosiak}.
	\textbf{(c)} Quadrant 1 of the back side of the CD using user calibration. A number of the radial strips have incorrect gains, as they don't lie on the straight line.
	See text for more information.}
	\label{fig:FBE}
\end{figure}
...
\subsection{Double peak structure}\label{ssec:DPS}
...
If the threshold is set too low, pedestal events are included in the \textcolor{red}{??? spectra / other stuff / Coulex something ???} and it will get things wrong. \textcolor{red}{Jeg vet ikke hva som går galt, dette var forklaringen jeg fikk fra Liam.....:(}
...
\begin{figure}[ht]
	...
	\caption{ADC time offsets for the four quadrants of the CD. See text for information.}
	\label{fig:ADC_dt}
\end{figure}
...
During the setup of the experiment, a hardware and software calibration of the \ga\ detectors was performed by the ISOLDE staff. 
...
However, there are non-linearities and drifting offsets and gains over time that might have to be corrected for with a proper calibration.
Therefore, a calibration run using \Ba\ and \Eu\ was performed at the end of the experiment.
The \Ba\ and \Eu\ sources were placed in the target position simultaneously, back to back.
...
The quality of this calibration has been carefully checked.
It was concluded to be very acceptable, meaning there was no need to improve on it. 
Even if the resolution had been improved by 0.1 keV for the source data, once the Doppler broadening gets into play, it makes no difference.
\autoref{fig:gamma_comparison} displays all 144 \ga\ detector segments plotted on top of each other. 
The alignment of the segments are very satisfactory.

\begin{figure}[ht]
	\centering
	\includegraphics[width=\textwidth]{../Plots/comparing/gamma_comparison_full.png}
	\caption{Comparison of all 144 \ga\ detector segments. 
	Each of the 8 clusters has a different color.}
	\label{fig:gamma_comparison}
\end{figure}

Similar to the particle calibration, the online \ga\ calibration was utilized in the present work. 
...
Therefore, many data points can be used in the calibration.
The fit can either be linear or quadratic, depending on the detector specification.
A simple calibration, with only two centroids can be performed similarly to the particle detector calibration.
From \autoref{eq:offset} and \autoref{eq:gain} we have more specifically
...
\end{align}
where $\Gamma$ can be either Eu or Ba.

In a similar manner to the particle detector calibration, the \ga\ detector calibration can be performed.
...
This script was never written, but the idea is the same as the particle calibration.
...
These scripts need to be modified if a new calibration is performed.
...
The next step is to run \texttt{CLXAna}.
\texttt{CLXAna} applies the Doppler correction in order to get the Doppler-corrected \ga-spectra to analyze the Coulomb excitation of \Sm.
...
When utilizing the add-back method, in some cases two individual \ga-rays are added together, making the sum peak stronger.
...
\autoref{sec:CLXAna} goes through the details of the graphical cuts.
...
Finished.
\end{lstlisting}
A detailed explanation of the input parameters is provided in \autoref{sec:CLXAna}.

\autoref{fig:part_wcut} shows the cuts of the beam and target for the detected particle events. 
% as the solid and dotted lines. 
...
\textcolor{Magenta}{Tilbakemelding: \newline
$\bullet$ Motivation: to explain what is known, and which transition probabilities you want to measure. Perhaps also to explain what theory predicts. \newline
...
From a previous experiment at the Heavy Ion Laboratory of the University of Warsaw, the spin values of the 990 keV and 1599 keV levels in \Sm\ was corrected to $2^+$ and $0^{(+)}$, respectively \cite{Klintefjord2015, Samorajczyk2015}.
% using the \ga-\ga\ angular correlation method \cite{Samorajczyk2015}. 
% using the angular correlation technique

From the previous experiment at ISOLDE, IS495 in 2012, three excited states of \Sm\ were populated, the first (531 keV) and second (990 keV) excited $2^+$ state, in addition to the first $4^+$ (1246 keV) state.
The $B(E2)$ values for the low-lying states was obtained \cite{Klintefjord2015, Klintefjord2016}.

\textcolor{RedViolet}{"The fact that \Sm\ has a very low-lying $2_2^+$ state supports the notion of triaxiality."} \cite{Klintefjord2016}

A lifetime measurement of the $2_1^+$ state also gave the $B(E2)$ value \cite{BelloGarrote2015}.
% using the \ga-\ga\ angular correlation method
...
\textcolor{RedViolet}{"It can be expected that more information about the nature of the excited 0+ states and the missing 3+ state will come from a planned Coulomb excitation experiment at the new high intensity energy (HIE) ISOLDE facility which is currently under construction at CERN."} \cite{Samorajczyk2015}


\bigskip

From the present experiment at ISOLDE, IS558 in 2017, it was expected that excited sates up to $8^+$ is populated. 
\autoref{tab:gamma_trans} displays the known \ga\ transitions for excited \Sm\ up to the $8^+$ state.
\autoref{fig:levels} displays the level scheme based on \autoref{tab:gamma_trans} for all known levels of \Sm\ up to the $8^+$ state at 2970 keV. 
Many of the states, but not all, are visible in \autoref{fig:gam_dcB}. 
There are different reasons for this, \ga\ transitions over 2000 keV are not visible because the histogram only shows energies up to this value.
...
\autoref{fig:gam_dcB} displays the \ga\ spectrum obtained from \texttt{CLXAna}. 
A lot of the \ga\ transitions in \autoref{fig:levels} are visible in the spectrum, marked by the black arrows.
The green arrows marks some of the energy levels in the level scheme, this is a combination of two \ga\ transitions, i.e. a summing of the peaks.
At very low energy, below the Compton continuum, a peak of \Pb\ x-rays are visible. 
\textcolor{red}{Hvorfor ser vi disse igjen?}

The transition probabilites, i.e. $B(E2)$, are relatively strong in the ground state band. 

The $0_2^+$ and $0_3^{(+)}$ states can tell us about the shape of the nucleus, in addition to if there is any shape coexistence. 



\begin{table}[htb] 
    \centering 
    \caption{Known \ga\ transitions based on \cite{Klintefjord, NNDC-levels}. 
    \textcolor{red}{Er kolonnetitlene selvforklarende eller må jeg si hva de er?}    
    The column "Visible in \ga\ spectrum" tells if it is possible to see the transition in \autoref{fig:gam_dcB}.}
...
\autoref{fig:projections} ..... the angle range can be split into 23 parts.
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{0.49\textwidth}
		...
		%\label{fig:???}
	\end{subfigure}
	\caption{\textbf{(a)-(d)} shows when the beam particle hits the CD with the LAB angle as specified in the plot title, while \textbf{(e)-(f)} shows when the target hits the CD with the LAB angle as specified in the plot title. T is for target and B is for beam. Combined they cover the angle range $\theta_b^{'} \in [36.6^\circ, 136.0^\circ]$ of the beam in the CM frame. \textcolor{red}{For å unngå overlapp valgte jeg (e) slik at den går fra 94.4 - 115.6. Hvis jeg går ned en vinkel-range til kommer den til 90 grader. Skal jeg la den vær slik, eller skal jeg gå ned til 90 grader, slik at det blir 1 grad overlapp mot (d)?}}
...
\chapter{MiniballCoulexSort and ROOT}
...
Line 358 and 359 in \texttt{kinsim3}, where the factor needs to be changed, reads
\begin{lstlisting}[language=c++]
