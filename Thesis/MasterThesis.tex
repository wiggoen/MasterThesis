...
At the suggestion of Ernest Rutherford did Hans Geiger and Ernest Marsden bombard $\alpha$-particles into a thin gold (metal) foil, and observed the scattering of the particles.
...
Around every magic number of neutrons or protons, the binding energy of a nucleus would fall. 
...
In addition to the nuclear shell model, there exist a multitude of different macroscopic and microscopic models to describe the nucleus. 
...
\begin{align}\label{eq:GPandA}
	g_p &= \frac{E_{\text{Sm}} - E_{\text{Pb}}}{n_{\text{Sm}} - n_{\text{Pb}}} &  a &= E_{Sm} - g_p \cdot n_{Sm}
\end{align}
where the $g_p$ is the gain for the particle, $a$ is the offset and $n$ is the channel number.
...
The gains and offsets of each DGF were provided by the ISOLDE staff in such a way that the online analysis is more straightforward.
However, there are non-linearities and drifting offsets and gains over time that have to be corrected for with a proper calibration.
...
In addition, the data from the calibration run can be used to determine the relative efficiency of the Miniball spectrometer. \textcolor{red}{omformuler denne setningen..}


\bigskip

In the calibration run, there might be many \ga\ lines in both Eu and Ba. 
...
\begin{align}\label{eq:GGandA}
	g_\gamma &= \frac{E_{\text{Eu}} - E_{\text{Ba}}}{n_{\text{Eu}} - n_{\text{Ba}}} &  a &= E_{Eu} - g_\gamma \cdot n_{Eu}
\end{align}
...
A \ga\ detector calibration was already performed during the experiment to be able to monitor the data on-line. 
...
\begin{figure}[ht]
	...
	\caption{Comparison of all 144 \ga\ detector segments. Each cluster has a different color.}
...
In a similar manner to the particle detector calibration, the ...
...
The next step is to do a Doppler correction, as explained in  
