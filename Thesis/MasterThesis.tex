...
Deformation leads to a more stable nuclei. The nucleus can win energy. \textcolor{red}{finn en passende plass..}

\subsection{Nuclear / quadrupole deformation}
A way to understand nuclear deformation is to imagine a liquid drop vibrating \textcolor{red}{??? and/or rotating ???} at high frequency. 
The average shape of the nucleus (liquid drop) is spherical, but the instantaneous shape is not.
...
\end{equation}
where $R_0$ is the radius of the sphere, $a_{00}$ describes volume changes, $a_{\lambda \mu}$ are \textcolor{red}{the expansion coefficients (or the deformation parameters) or the amplitude of} the spherical harmonics $Y_{\lambda \mu}$, $\lambda$ is the multipole, $\mu$ is the order of the expansion and $\theta$ and $\phi$ are the intrinsic angles \cite{Krane, RS, Klintefjord}. 

\begin{figure}[ht]
	\centering
	\includegraphics[width=0.6\textwidth]{Images/vibrating-nucleus.png}
	\caption{A deformed vibrating nucleus, adapted from \cite{Krane}. The dotted lines represent the spherical equilibrium shape. See text for more information.}
...
Other conditions is that $\textcolor{red}{??? R / \mathbf{R} ???}$ should be invariant under reflection and rotation of the coordinate system. 
That means $R$ should be unchanged by transformations of the coordinate system.
...
These special parameters, $a_{\lambda 0}$, are called $\beta_2$.

\autoref{fig:vibrational-modes} shows a sketch of the three lowest vibrational modes. 
The quadrupole vibration, $\lambda = 2$, is the most important mode.
It describes the shape of the nucleus and is the dominant feature in almost all (\textcolor{red}{deformed?}) nuclei. 
...
For $\lambda = 2$, there is five parameters of $a_{2\mu}$. 
Two parameters describe the shape and the three others describe the orientation in space. %(Euler angles $\Omega = (\alpha, \beta, \gamma)$). 
It is possible to align the deformed shape in a coordinate system such that only two parameters are needed to describe the system, the shape of the nucleus.
...
\autoref{fig:Nuclear-shapes} shows a sketch of three different deformed nuclear shapes. \textcolor{red}{denne ble litt overflødig når jeg fikk inn formene i fig. 2.4.}
...
%	\label{fig:Lund-convention}
%\end{figure}

\begin{figure}[htb]
	\centering
	\includegraphics[width=0.8\textwidth]{Images/nuclear-shapes.png}
	\caption{Nuclear shapes, adapted from \cite{MSU-shapes}. 
	(a) The prolate shape looks like an American football. 
	(b) The oblate shape looks like a pumpkin. 
	(c) The triaxial shape looks like a kiwi fruit. 
	%It is at $30^\circ$ in \autoref{fig:Lund-convention} using the Lund convention, between the prolate and the oblate shape.
	}
...
\begin{figure}[ht]
	...
	\caption{Collective excitations for \Sm, adapted from \cite{Hilaire2007, CEA}. \textcolor{red}{\Sm\ looks like a \ga-soft vibrator? See lecture FYS4570 18. Sept 2018.}}
	\label{fig:cea_tri}
\end{figure}
...
\begin{figure}[htb]
	...
	\caption{Chart of nuclides for deformed nuclei, from \cite{Hilaire2007, CEA}. \Sm\ is inside the pink ring, to the left of the yellow square.}
	\label{fig:cea_CoN}
\end{figure}
...
\subsection{Transition probability (\textcolor{red}{cross sections?})}
...
\begin{figure}[ht]
	...
	\caption{Symmetry triangle, from \cite{Casten2006}. \textcolor{red}{Det er ikke sikkert denne skal være med.}}
