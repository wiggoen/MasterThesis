...
function usage {
    ...
    echo "e.g. '-s' to run Coulomb excitation analysis on data."
    echo "Argument flag choice: '-d' or '-s' (singles), '-a' (addback), '-r' (reject)."
...
if [ $# != 1 ]; then
    ...
    case "$1" in
        -d)
            # The default option is in fact the same as the singles option
